...
>
    <h2 class="font-semibold text-2xl mb-5 mt-5">Address</h2>
    ...
    <div class="mb-2 m-2">
        ...
        {:else}
            <select bind:value={country_code} on:change={({ target }) =>  country_code = target.value}
                    ...
                    name="country">
                {#each cart.countries as {display_name, iso_2}}
                    <option value={iso_2}>
                        {display_name}
                    </option>
                {/each}
    ...
    <button type="submit" class="custom-btn mb-2 m-2" style="width: 98%" > Next</button>
</Form>
